...
{
    class MyLitSettingsView : MasterNodeSettingsView
    {
        MyLitMasterNode m_Node;
        ...
        public MyLitSettingsView(MyLitMasterNode node) : base(node)
        {
            m_Node = node;
            ...
            if (m_Node.surfaceType == SurfaceType.Transparent)
            {
                ...
                // No refraction in MyLit, always show this:
                ps.Add(new PropertyRow(CreateLabel("Blending Mode", indentLevel)), (row) =>
                {
                    row.Add(new EnumField(MyLitMasterNode.AlphaModeLit.Additive), (field) =>
                    {
            ...
            {
                row.Add(new EnumField(MyLit.BaseParametrization.BaseMetallic), (field) =>
                {
            ...
            if (m_Node.baseParametrization == MyLit.BaseParametrization.SpecularColor)
            {
                ++indentLevel;
            ...
            if (m_Node.dualSpecularLobe.isOn)
            {
                ...
                {
                    row.Add(new EnumField(MyLit.DualSpecularLobeParametrization.HazyGloss), (field) =>
                    {
                ...
                if ((m_Node.baseParametrization == MyLit.BaseParametrization.BaseMetallic)
                    && (m_Node.dualSpecularLobeParametrization == MyLit.DualSpecularLobeParametrization.HazyGloss))
                {
            ...
            if (m_Node.devMode.isOn)
            {
                ...
                {
                    row.Add(new EnumField(MyLitMasterNode.SpecularOcclusionBaseMode.DirectFromAO), (field) =>
                    {
                ...
                if (MyLitMasterNode.SpecularOcclusionModeUsesVisibilityCone(m_Node.screenSpaceSpecularOcclusionBaseMode))
                {
                    ++indentLevel;
                    ps.Add(new PropertyRow(CreateLabel("Specular Occlusion (SS) AO Cone Weight", indentLevel)), (row) =>
                    {
                        row.Add(new EnumField(MyLitMasterNode.SpecularOcclusionAOConeSize.CosWeightedAO), (field) =>
                        {
                    ...
                    {
                        row.Add(new EnumField(MyLitMasterNode.SpecularOcclusionAOConeDir.ShadingNormal), (field) =>
                        {
            ...
            // SpecularOcclusion from input AO (baked or data-based SO)
            {
                ps.Add(new PropertyRow(CreateLabel("Specular Occlusion (from input AO)", indentLevel)), (row) =>
                {
                    ...
                    {
                        row.Add(new EnumField(MyLitMasterNode.SpecularOcclusionBaseModeSimple.DirectFromAO), (field) =>
                        {
                            // In non-dev mode, parse any enum value set to a method not shown in the simple UI as SPTD (highest quality) method:
                            MyLitMasterNode.SpecularOcclusionBaseModeSimple simpleUIEnumValue =
                                Enum.TryParse(m_Node.dataBasedSpecularOcclusionBaseMode.ToString(), out MyLitMasterNode.SpecularOcclusionBaseModeSimple parsedValue) ?
                                    parsedValue : MyLitMasterNode.SpecularOcclusionBaseModeSimple.SPTDIntegrationOfBentAO;
                            field.value = simpleUIEnumValue;
                            field.RegisterValueChangedCallback(ChangeDataBasedSpecularOcclusionBaseModeSimpleUI);
                        });
                    }
                });
                if (MyLitMasterNode.SpecularOcclusionModeUsesVisibilityCone(m_Node.dataBasedSpecularOcclusionBaseMode))
                {
                    ++indentLevel;
                    ps.Add(new PropertyRow(CreateLabel("Specular Occlusion AO Cone Weight", indentLevel)), (row) =>
                    {
                        row.Add(new EnumField(MyLitMasterNode.SpecularOcclusionAOConeSize.CosWeightedBentCorrectAO), (field) =>
                        {
            ...
            if (m_Node.SpecularOcclusionUsesBentNormal())
            {
                if (m_Node.devMode.isOn)
                {
                    ...
                    {
                        row.Add(new EnumField(MyLitMasterNode.SpecularOcclusionConeFixupMethod.Off), (field) =>
                        {
                ...
                else
                {
                    ...
                    ps.Add(new PropertyRow(CreateLabel("Specular Occlusion Bent Cone Fixup", indentLevel)), (row) =>
                    {
                        row.Add(new Toggle(), (toggle) =>
                        {
                            toggle.value = m_Node.specularOcclusionConeFixupMethod != MyLitMasterNode.SpecularOcclusionConeFixupMethod.Off;
            ...
            if (m_Node.devMode.isOn)
            {
                ps.Add(new PropertyRow(CreateLabel("Show And Enable MyLit Debugs", indentLevel)), (row) =>
                {
                    row.Add(new Toggle(), (toggle) =>
                    {
        ...
        void ChangeBaseParametrization(ChangeEvent<Enum> evt)
        {
            ...
            m_Node.baseParametrization = (MyLit.BaseParametrization)evt.newValue;
        }
        ...
        void ChangeDualSpecularLobeParametrization(ChangeEvent<Enum> evt)
        {
            ...
            m_Node.dualSpecularLobeParametrization = (MyLit.DualSpecularLobeParametrization)evt.newValue;
        }

        void ChangeBlendMode(ChangeEvent<Enum> evt)
        {
            // Make sure the mapping is correct by handling each case.
            AlphaMode alphaMode = GetAlphaMode((MyLitMasterNode.AlphaModeLit)evt.newValue);
        ...
        void ChangeScreenSpaceSpecularOcclusionBaseMode(ChangeEvent<Enum> evt)
        {
            ...
            if (Equals(evt.newValue, MyLitMasterNode.SpecularOcclusionBaseMode.Custom))
            {
            ...
            m_Node.screenSpaceSpecularOcclusionBaseMode = (MyLitMasterNode.SpecularOcclusionBaseMode)evt.newValue;
        }
        ...
        void ChangeScreenSpaceSpecularOcclusionAOConeSize(ChangeEvent<Enum> evt)
        {
            ...
            m_Node.screenSpaceSpecularOcclusionAOConeSize = (MyLitMasterNode.SpecularOcclusionAOConeSize)evt.newValue;
        }
        ...
        void ChangeScreenSpaceSpecularOcclusionAOConeDir(ChangeEvent<Enum> evt)
        {
            ...
            m_Node.screenSpaceSpecularOcclusionAOConeDir = (MyLitMasterNode.SpecularOcclusionAOConeDir)evt.newValue;
        }
        ...
        void ChangeDataBasedSpecularOcclusionBaseMode(ChangeEvent<Enum> evt)
        {
            ...
            m_Node.dataBasedSpecularOcclusionBaseMode = (MyLitMasterNode.SpecularOcclusionBaseMode)evt.newValue;
        }

        void ChangeDataBasedSpecularOcclusionBaseModeSimpleUI(ChangeEvent<Enum> evt)
        {
            // MyLitMasterNode.SpecularOcclusionBaseModeSimple should always be a subset of MyLitMasterNode.SpecularOcclusionBaseMode:
            if (Equals(m_Node.dataBasedSpecularOcclusionBaseMode, (MyLitMasterNode.SpecularOcclusionBaseMode) evt.newValue))
                return;
        ...
        void ChangeDataBasedSpecularOcclusionAOConeSize(ChangeEvent<Enum> evt)
        {
            ...
            m_Node.dataBasedSpecularOcclusionAOConeSize = (MyLitMasterNode.SpecularOcclusionAOConeSize)evt.newValue;
        }
        ...
        void ChangeSpecularOcclusionConeFixupMethod(ChangeEvent<Enum> evt)
        {
            ...
            m_Node.specularOcclusionConeFixupMethod = (MyLitMasterNode.SpecularOcclusionConeFixupMethod)evt.newValue;
        }

        void ChangeSpecularOcclusionConeFixupMethodSimpleUI(ChangeEvent<bool> evt)
        {
            if ( (evt.newValue == false && Equals(m_Node.specularOcclusionConeFixupMethod, MyLitMasterNode.SpecularOcclusionConeFixupMethod.Off))
                || (evt.newValue == true && Equals(m_Node.specularOcclusionConeFixupMethod, MyLitMasterNode.SpecularOcclusionConeFixupMethod.BoostAndTilt)) )
                return;

            m_Node.owner.owner.RegisterCompleteObjectUndo("SpecularOcclusionConeFixupMethod Change");

            m_Node.specularOcclusionConeFixupMethod = evt.newValue ? MyLitMasterNode.SpecularOcclusionConeFixupMethod.BoostAndTilt
                                                                     : MyLitMasterNode.SpecularOcclusionConeFixupMethod.Off;
        }
        ...
        {
            m_Node.owner.owner.RegisterCompleteObjectUndo("MyLit DevMode Change");
        ...
        {
            m_Node.owner.owner.RegisterCompleteObjectUndo("MyLit Debug Change");
        ...
        public AlphaMode GetAlphaMode(MyLitMasterNode.AlphaModeLit alphaModeLit)
        {
            switch (alphaModeLit)
            {
                case MyLitMasterNode.AlphaModeLit.Alpha:
                    return AlphaMode.Alpha;
                case MyLitMasterNode.AlphaModeLit.Premultiply:
                    return AlphaMode.Premultiply;
                case MyLitMasterNode.AlphaModeLit.Additive:
        ...
        public MyLitMasterNode.AlphaModeLit GetAlphaModeLit(AlphaMode alphaMode)
        {
            switch (alphaMode)
            {
                case AlphaMode.Alpha:
                    return MyLitMasterNode.AlphaModeLit.Alpha;
                case AlphaMode.Premultiply:
                    return MyLitMasterNode.AlphaModeLit.Premultiply;
                case AlphaMode.Additive:
                    return MyLitMasterNode.AlphaModeLit.Additive;
                default:
                    {
